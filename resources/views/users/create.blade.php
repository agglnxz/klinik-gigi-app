...
<div class="bg-white rounded-2xl shadow-md border border-gray-100 p-8 max-w-5xl">
    ...
    <form action="{{ route('users.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            <label for="name" class="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">Nama Akun / Nama Lengkap</label>
            <div class="relative">
                <span class="absolute inset-y-0 left-0 flex items-center pl-4 text-gray-400">
                    <i class="fa-solid fa-user"></i>
                </span>
                <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="Contoh: drg. Ahmad Faisal"
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input type="email" id="email" name="email" value="{{ old('email') }}" placeholder="Contoh: ahmad@klinikwinardi.com"
        ...
        <div>
            ...
            <div class="flex items-center space-x-6 bg-gray-50 border border-gray-200 p-3.5 rounded-xl">
                <label class="flex items-center space-x-2 text-xs font-semibold text-gray-700 cursor-pointer">
                    <input type="radio" name="is_aktif" value="1" {{ old('is_aktif', '1') == '1' ? 'checked' : '' }} class="w-4 h-4 text-[#429A80] focus:ring-[#429A80] border-gray-300">
                    <span>Langsung Aktif</span>
                </label>
                <label class="flex items-center space-x-2 text-xs font-semibold text-gray-600 cursor-pointer">
                    <input type="radio" name="is_aktif" value="0" {{ old('is_aktif') == '0' ? 'checked' : '' }} class="w-4 h-4 text-[#429A80] focus:ring-[#429A80] border-gray-300">
