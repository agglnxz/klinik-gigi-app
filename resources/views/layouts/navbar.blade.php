<nav class="bg-white p-4 flex justify-between items-center w-full border-b border-gray-100 shadow-sm relative z-30">
    ...
    <div class="flex items-center space-x-4">
        ...
        <div class="relative" x-data="{ open: false }">
            ...
            {{-- TOMBOL LONCENG --}}
            <button @click="open = !open" type="button" class="relative focus:outline-none flex items-center">
            ...
            >
                ...
                <div class="max-h-[350px] overflow-y-auto custom-scrollbar">
                    @forelse($notifikasiNavbar as $notif)
                        ...
                        <a
                            href="{{ route('pemesanan.show', $notif->pemesanan_id) }}"
                        ...
                        >
                            ...
                            <div class="flex-1">
                                <p class="text-xs font-bold text-gray-800">
                                    Pemesanan atas nama
                                    {{ $notif->pemesanan->pemeriksaan->pasien->nama ?? 'Pasien' }}
                                </p>

                                <p class="text-[11px] {{ $textColor }} mt-0.5 font-medium">
                                    {{ $notif->pesan }}
                                </p>
        ...
        <div class="relative" x-data="{ openProfile: false }">
            <button
                ...
                type="button"
                class="flex items-center space-x-1.5 focus:outline-none group"
            >
                <img
                    src="{{ Auth::user() && Auth::user()->foto
                        ? asset('storage/' . Auth::user()->foto) . '?v=' . (Auth::user()->updated_at ? Auth::user()->updated_at->timestamp : time())
