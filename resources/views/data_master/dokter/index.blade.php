...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-end">
            <div>
                <h3 class="text-2xl font-bold text-black-1000">Daftar Tenaga Medis (Dokter)</h3>
                <p class="text-sm text-gray-500 font-light">Kelola data tenaga medis dan informasi kontak dokter aktif Anda.</p>
            </div>
            <a href="{{ route('dokter.create') }}">
                <button
                    ...
                    <i class="fa-solid fa-plus text-xs"></i> Tambah Dokter Baru
                </button>
            </a>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
                ...
                <table class="w-full text-left border-collapse min-w-[1000px]">
                    <thead>
                        <tr class="bg-[#F3F4F3] border-b border-gray-100">
                            <th class="w-[300px] px-8 py-4 text-[11px] font-bold text-bold-900 uppercase tracking-widest">
                                Nama Dokter</th>
                            <th class="w-[250px] px-8 py-4 text-[11px] font-bold text-bold-900 uppercase tracking-widest">
                                Kontak / No. Telepon</th>
                            <th class="w-[200px] px-8 py-4 text-[11px] font-bold text-bold-900 uppercase tracking-widest">
                                Status Akun</th>
                            <th class="w-[250px] px-8 py-4 text-[11px] font-bold text-bold-900 uppercase tracking-widest">
                                Aksi</th>
                        </tr>
                            ...
                            <tr class="hover:bg-gray-50/30 transition">
                                <td class="px-8 py-6">
                                    <div class="flex items-center space-x-3">
                                        <div class="w-10 h-10 rounded-lg bg-teal-100 text-teal-700 flex items-center justify-center font-bold text-xs uppercase">
                                            DR
                                        </div>
                                        <div>
                                ...
                                <td class="px-8 py-6 text-gray-500 font-medium">{{ $item->kontak }}</td>
                                <td class="px-8 py-6">
                                    @if ($item->is_aktif)
                                ...
                                <td class="px-8 py-6">
                                    <div class="flex space-x-2">
                                        ...
                                        @else
                                            ...
                                            <button type="button"
                                                @click="$dispatch('buka-modal-hapus', { id: '{{ $item->id }}', nama: '{{ $item->nama }}', tabel: 'dokter' })"
            ...
            <div class="bg-[#F3F4F3] px-8 py-4 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                <p class="text-[11px] font-normal text-gray-400 italic">
                    Menampilkan {{ $dokter->count() }} data dokter aktif saat ini.
                </p>
            </div>
        </div>
    </div>

    {{-- MODAL BOX ALPINJS UNTUK INPUT ALASAN HAPUS --}}
    <div x-data="{ open: false, idReferensi: '', namaData: '', namaTabel: '', alasan: '' }"
        ...
        <div @click.outside="open = false"
            ...
            <div class="flex items-center gap-3 text-red-500 mb-4">
                ...
                <h3 class="text-base font-bold text-gray-800">Pengajuan Hapus Data Dokter</h3>
            </div>

            <p class="text-xs text-gray-500 mb-4 leading-relaxed">
                Anda mengajukan penghapusan untuk dokter bernama: <span class="font-bold text-gray-800" x-text="namaData"></span>. Data tidak akan langsung terhapus sebelum disetujui oleh Direktur.
            </p>
            ...
            <form :action="'{{ route('pengajuan-hapus.store') }}'" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="alasan_hapus" x-model="alasan" required rows="3"
                        placeholder="Tulis alasan administratif atau rotasi kerja mengapa dokter ini diajukan hapus..."
