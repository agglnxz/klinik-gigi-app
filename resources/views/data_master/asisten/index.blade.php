...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-end mb-6">
        <div>
            ...
            <p class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Total {{ $asisten->count() }} Asisten</p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
            ...
            <table class="w-full text-left border-collapse min-w-[1000px]">
                <thead>
                    <tr class="bg-[#F3F4F3] border-b border-gray-100">
                        <th class="w-[300px] px-8 py-4 text-[11px] font-bold text-bold-900 uppercase tracking-widest">Nama Asisten</th>
                        <th class="w-[250px] px-8 py-4 text-[11px] font-bold text-bold-900 uppercase tracking-widest">Nomor HP</th>
                        <th class="w-[200px] px-8 py-4 text-[11px] font-bold text-bold-900 uppercase tracking-widest">Status</th>
                        <th class="w-[250px] px-8 py-4 text-[11px] font-bold text-bold-900 uppercase tracking-widest">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-50 text-sm">
                    @forelse($asisten as $item)
                        ...
                        <td class="px-8 py-6">
                            <div class="flex space-x-2">
                                ...
                                @else
                                    ...
                                    <a href="{{ route('asisten.edit', $item->id) }}"
                                    ...
                                    <button type="button"
                                        @click="$dispatch('buka-modal-hapus', { id: '{{ $item->id }}', nama: '{{ $item->nama }}', tabel: 'asisten' })"
                    ...
                    <tr>
                        <td colspan="4" class="px-8 py-20 text-center">
                            <div class="flex flex-col items-center opacity-40">
                                ...
                                <p class="text-sm font-medium">Belum ada data asisten yang terdaftar</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div class="bg-[#F3F4F3] px-8 py-4 border-t border-gray-100">
            <p class="text-[11px] text-gray-400 font-medium text-left">Menampilkan {{ $asisten->count() }} Asisten Saat Ini</p>
        </div>
    </div>
</div>
...
<div x-data="{ open: false, idReferensi: '', namaData: '', namaTabel: '', alasan: '' }"
    ...
    <div @click.outside="open = false"
        ...
        <div class="flex items-center gap-3 text-red-500 mb-4">
            ...
            <h3 class="text-base font-bold text-gray-800">Pengajuan Hapus Data Asisten</h3>
        </div>

        <p class="text-xs text-gray-500 mb-4 leading-relaxed">
            Anda mengajukan penghapusan untuk asisten bernama: <span class="font-bold text-gray-800" x-text="namaData"></span>. Data tidak akan langsung terhapus sebelum disetujui oleh Direktur.
        </p>
        ...
        <form :action="'{{ route('pengajuan-hapus.store') }}'" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="alasan_hapus" x-model="alasan" required rows="3"
                    placeholder="Tulis alasan administratif mengapa asisten ini diajukan hapus..."
