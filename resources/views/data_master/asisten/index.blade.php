...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-end mb-6">
        <div>
            ...
            <p class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Total Total {{ $asisten->count() }} Asisten</p>
        </div>
                        ...
                        <td class="px-8 py-6">
                            <div class="flex space-x-2">
                                <a href="{{ route('asisten.edit', $item->id) }}">
                                    <button class="flex items-center px-4 py-2 bg-[#59a38a] text-white text-xs font-bold rounded-md hover:bg-[#46826d] transition shadow-sm hover:shadow">
                                ...
                                <form action="{{ route('dokter.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menonaktifkan dokter ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="flex items-center px-4 py-2 bg-[#d65f5f] text-white text-xs font-bold rounded-md hover:bg-[#b54d4d] transition shadow-sm hover:shadow">
